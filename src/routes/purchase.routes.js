...
const { validatePR, validatePO, validateGRN } = require('../middleware/validator.middleware');

router.use(protect);

// Purchase Requests
router.get('/purchase-requests', purchase.getAllPRs);
router.post('/purchase-requests', validatePR, purchase.createPR);
...
router.post('/purchase-orders', validatePO, purchase.createPO);
...
router.post('/grns', validateGRN, purchase.createGRN);
